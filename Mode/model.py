...
# from BiGRU2 import Seq2SeqEncoder
...
class EncoderLayer(nn.Module):
    def __init__(self, d_model, d_k, n_heads, d_v, d_ff, method_max_length, source_max_length, device):
        super(EncoderLayer, self).__init__()
        self.enc_self_attn = MultiHeadAttention(d_model, d_k, n_heads, d_v, device)
        
        self.gating = Gating(d_model, method_max_length, source_max_length)
        self.pos_ffn = PoswiseFeedForwardNet(d_model, d_ff, device)

    def forward(self, enc_inputs, method_inputs, enc_self_attn_mask):
        '''
        ...
        enc_outputs = self.gating(enc_outputs, method_inputs)
        enc_outputs = self.pos_ffn(enc_outputs)  # enc_outputs: [batch_size, src_len, d_model]
        return enc_outputs, attn


class Encoder(nn.Module):
    def __init__(self, src_vocab_size, d_model, dropout, n_layers, d_k, n_heads, d_v, d_ff, method_max_length, source_max_length, device):
        super(Encoder, self).__init__()
        self.src_vocab_size = src_vocab_size
        self.src_emb = nn.Embedding(self.src_vocab_size, d_model)
        self.pos_emb = PositionalEncoding(d_model, dropout)
      
        self.layers = nn.ModuleList([EncoderLayer(d_model, d_k, n_heads, d_v, d_ff, method_max_length, source_max_length, device) for _ in range(n_layers)])

    def forward(self, enc_inputs, method_inputs):
        '''
        ...
        for layer in self.layers:
            ...
            enc_outputs, enc_self_attn = layer(enc_outputs, method_inputs, enc_self_attn_mask)
...
class MultiDecoderLayer(nn.Module):
    def __init__(self, d_model, d_k, n_heads, d_v, d_ff, device):
        ...
        # self.dec_method_attn = MultiHeadAttention1(d_model, d_k, n_heads, d_v, device)
        self.pos_ffn = PoswiseFeedForwardNet(d_model, d_ff, device)
    ...
    def forward(self, dec_inputs, enc_outputs, multi_outputs, dec_self_attn_mask, dec_enc_attn_mask):
        ...
        # dec_outputs, dec_method_attn = self.dec_method_attn(dec_outputs, method_outputs, method_outputs)
...
class MultiDecoder(nn.Module):
    ...
    def forward(self, dec_inputs, enc_inputs, enc_outputs, ast_outputs, ast_embed):   
        '''
        ...
        dec_ast_attns, dec_method_attns = [], []
        for layer in self.layers:
            ...
            # dec_method_attns.append(dec_method_atten)
        return dec_outputs, dec_self_attns, dec_enc_attns, dec_ast_attns


class Model(nn.Module):
    def __init__(self, src_vocab_size, tgt_vocab_size, max_ast_node, src_max_length, method_max_length, nfeat, nhid, nout, d_model, batch_size, dropout, d_k, d_v, d_ff, n_heads, n_layers, device):
        super(Model, self).__init__()
        self.encoder = Encoder(src_vocab_size, d_model, dropout, n_layers, d_k, n_heads, d_v, d_ff, method_max_length, src_max_length, device).to(device)
        # self.gru = Seq2SeqEncoder(d_model, d_model)
    ...
    def forward(self, enc_inputs, method_inputs, dec_inputs, x, a1, a2, a3, a4, a5):    #
        '''
        enc_inputs: [batch_size, src_len]
        dec_inputs: [batch_size, tgt_len]
        '''
        ...
        enc_outputs, enc_self_attns = self.encoder(enc_inputs, method_inputs)
        # method_outputs = self.gru(method_inputs)
