...
class MultiDecoderLayer(nn.Module):
    ...
    def forward(self, dec_inputs, enc_outputs, multi_outputs, method_outputs, dec_self_attn_mask, dec_enc_attn_mask):
        '''
        ...
        return dec_outputs, dec_self_attn, dec_enc_attn, dec_ast_attn, dec_method_attn
...
class MultiDecoder(nn.Module):
    ...
    def forward(self, dec_inputs, enc_inputs, enc_outputs, ast_outputs, method_outputs, ast_embed):   # 变动
        '''
        ...
        for layer in self.layers:
            ...
            dec_outputs, dec_self_attn, dec_enc_attn, dec_ast_attn, dec_method_atten = layer(dec_outputs,  enc_outputs, multi_model_out, method_outputs, dec_self_attn_mask, dec_enc_attn_mask)
            dec_self_attns.append(dec_self_attn)
            dec_enc_attns.append(dec_enc_attn)
            dec_ast_attns.append(dec_ast_attn)
            dec_method_attns.append(dec_method_atten)
        return dec_outputs, dec_self_attns, dec_enc_attns, dec_ast_attns, dec_method_attns
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, dec_inputs, enc_outputs, method_outputs, dec_self_attn_mask, dec_enc_attn_mask):
        '''
        dec_inputs: [batch_size, tgt_len, d_model]
        enc_outputs: [batch_size, src_len, d_model]
        dec_self_attn_mask: [batch_size, tgt_len, tgt_len]
...
class Model(nn.Module):
    ...
    def forward(self, enc_inputs, method_inputs, dec_inputs, x, a1, a2, a3, a4, a5):    # 变动
        '''
        enc_inputs: [batch_size, src_len]
        dec_inputs: [batch_size, tgt_len]
        '''
        ...
        dec_outputs1, dec_self_attns, dec_enc_attns, dec_ast_attns, dec_method_attns = self.decoder1(dec_inputs, enc_inputs, enc_outputs, ast_outputs, method_outputs, ast_embed)   # 变动
        ...
        return dec_logits1.view(-1, dec_logits1.size(-1)), enc_self_attns, dec_self_attns, dec_enc_attns, dec_ast_attns, dec_method_attns
